...
```powershell
cd c:\Users\de0188442\source\repos\browser-cli
```
...
### 1. Uso Padrão (Modo Leitura e Abas no Terminal)
```powershell
python main.py https://news.ycombinator.com
```

### 2. Leitura de Artigos com Paginação (recomendado para páginas longas)
```powershell
...
```powershell
python main.py "https://docs.python.org/3/tutorial/controlflow.html"
```
...
| Argumento | Descrição |
|-----------|-----------|
| `url` | URL do site a ser acessado (ex: `example.com` ou `https://news.ycombinator.com`). |
